...
Не взято в этот слайс: 6-й пункт группы E, глобальный переключатель «Hide PowerShell windows during setup» — требует протащить `hidden bool` через ~10 функций в 4 файлах (`scripts.go`, `optimizations.go`, `startmenu.go`, `vmapplocker.go`), инвазивный рефакторинг сигнатур, отложен отдельным пунктом бэклога.
...
**Группа E — прочие setup-settings и мелкие механизмы. ЗАКРЫТА в слайсе 22, кроме первого пункта.**
- Глобальный переключатель «Hide PowerShell windows during setup» (`-WindowStyle Hidden` вместо `Normal` для всех PS-скриптов сразу, не только пользовательских) — требует протащить `hidden bool` через ~10 функций в 4 файлах (`scripts.go`, `optimizations.go`, `startmenu.go`, `vmapplocker.go`) + сигнатуры `NewDeployment`/`NewShellSetupOOBE`, инвазивный рефакторинг, сознательно отложен.
