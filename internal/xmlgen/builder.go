...
func BuildAnswerFile(p *profile.Profile) (string, error) {
	...
	if deployment := components.NewDeployment(p.SystemTweaks, p.BypassOnlineAccountRequirement, p.PasswordExpiration, p.AccountLockout, p.FileExplorer, p.Personalization, p.RemoveApps, p.StickyKeys, p.LockKeys, p.SystemScripts, p.DefaultUserScripts, p.UserOnceScripts); deployment != nil {
