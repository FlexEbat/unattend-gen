...
func BuildAnswerFile(p *profile.Profile) (string, error) {
	...
	if deployment := components.NewDeployment(p.SystemTweaks, p.BypassOnlineAccountRequirement, p.PasswordExpiration, p.AccountLockout, p.SystemScripts, p.DefaultUserScripts, p.UserOnceScripts); deployment != nil {
	...
	if shellOOBE := components.NewShellSetupOOBE(p.Accounts, p.FirstLogon, p.ExpressSettings, p.Wifi, p.BypassOnlineAccountRequirement, p.RemoveApps, p.RemoveFeatures, p.FirstLogonScripts, p.RestartExplorerAfterScripts); shellOOBE != nil {
